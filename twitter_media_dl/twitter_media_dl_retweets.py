...
BASE_DIR = "TwitterRetweets"
HISTORY_FILE = "rt_urls"
...
def get_media_details(tweets: TweetList) -> List[Dict[str, Union[str, int]]]:
    ...
    for tweet in tweets:
        try:
            ...
            for media_index, media in enumerate(tweet.retweeted_status.extended_entities.media, 1):
...
async def download_file(filename: str, media_details: Dict[str, str], 
                        session: aiohttp.ClientSession, new_session: bool =False) -> None:
    if new_session:
        session = aiohttp.ClientSession()
    # 2 final locations
    all_folder = f"{BASE_DIR}\\__all__\\{filename}"
    artist_folder = f"{BASE_DIR}\\{media_details['author']}\\{filename}"
    ...
    while retry_count <= 5:
        ...
        try:
            ...
            os.makedirs(f"{BASE_DIR}\\__all__\\", exist_ok=True)
            os.makedirs(f"{BASE_DIR}\\{media_details['author']}\\", exist_ok=True)
...
class ImageDownloadClient(peony.PeonyClient):
    ...
    def startup(self, queue_maxsize: int =50) -> None:
        ...
        try:
            filename = glob.glob(f"{HISTORY_FILE}*.txt")[0]
            with open(filename, "r") as f:
                self.img_urls.update(line.strip() for line in f.readlines())
        except IndexError:
            warnings.warn(f"No {HISTORY_FILE}.txt file found! Continuing without a history log...")
    ...
    async def add_media_to_queue(self, user_id: int =TWITTER_ID, count: int =300, max_tweets: int =4000) -> None:
        ...
        request = self.api.statuses.user_timeline.get(id=user_id, count=count, tweet_mode="extended", include_rts=True)
...
if __name__ == "__main__":
    try:
        ...
        print("Saving data...")
        # Move current img_urls to folder
        os.makedirs(f"{HISTORY_FILE}_backups", exist_ok=True)
        current_img_urls = glob.glob(f".\\{HISTORY_FILE}-*.txt")
        for img_url_file in current_img_urls:  # could be multiple files?? shouldn't be though
            os.rename(img_url_file, f"{HISTORY_FILE}_backups\\{img_url_file}")
        
        # Save current version
        current_time = datetime.datetime.now().strftime("%Y-%m-%d-%H%M%S")
        new_filename = f"{HISTORY_FILE}-{current_time}.txt" # Make new version's filename
